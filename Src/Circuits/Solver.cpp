...
{
	string s;
	cout<<"Enter INPUT file name: "; cin>>s;
	input.open(s+".txt");
	if(input.is_open())
	{
		input>>s;
		...
		while(!input.eof())
		{
			...
			if(s=="Vs")
			{
				...
				VScource.voltage = complex<double>(magn * cos(ph*PI/180), magn * sin(ph*PI/180));//polar(magn, ph*PI/180);
			...
			else if(s == "Cs")
			{
				...
				double magn = stod(mag);
				double ph = stod(phase);
				SScource.current = complex<double>(magn * cos(ph*PI/180), magn * sin(ph*PI/180));//polar(magn, ph*PI/180);
			...
			else if(s[0] == 'R')
			{
				...
				R.node1 = stoi(n1);
				R.node2 = stoi(n2);
				float magn = stof(mag);// stod(mag);
				R.Z = complex<double>(magn, 0);
				R.Y = pow(R.Z, -1);//polar(1/magn, 0);
				R.factor = 0;
...
int main()
{
	...
	for(int i=0; i<components.size(); ++i)
	{
		...
		else if(components[i].node1 !=0 && components[i].node2 !=0 && components[i].name == "Vs") //same as previus but we have two additional equations
		{
			Ys(components[i].node1-1, components[i].VS_num-1) += complex<double>(1,0);
			Ys(components[i].node2-1, components[i].VS_num-1) += complex<double>(-1,0);
			Ys(components[i].VS_num-1, components[i].node1-1) += complex<double>(1,0);
		...
		else if(components[i].name[0] == 'R' || components[i].name[0] == 'C' || components[i].name[0] == 'L')
		{
	...
	for (; i < nodes_num; i++)
	{
		cout << "V(" << i + 1 << ")" << " " << abs(IS(i, 0)) << " " << (arg(IS(i, 0))*(180 / 3.141592654));
		cout << endl;                                                            
	}
	for (int j = 0; j < nodes_num; j++)
	{
		...
		{
			cout << "I(" << components[j].node1 << " " << components[j].node2 << ")" << " " << abs(IS(i, 0)) << " " << (arg(IS(i, 0))*(180 / 3.141592654));
			i++;
			cout << endl;
		}
		...
		if (components[j].name[0] == 'R' || components[j].name[0] == 'L' || components[j].name[0] == 'C')
		{
			if (components[j].node1 != 0 && components[j].node1 != 0)
			{
				complex<double> I = ((IS(components[j].node1 - 1, 0) - IS(components[j].node2 - 1, 0)) / components[j].Z);
			...
			else
			{
				...
				{
					complex<double> I = ((complex<double>(0, 0) - IS(components[j].node2 - 1, 0)) / components[j].Z);
					cout << "I(" << components[j].node1 << " " << components[j].node2 << ")" << " " << abs(I) << " " << (arg(I)*(180 / 3.141592654));
					cout << endl;
				}
